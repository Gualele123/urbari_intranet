...
$select_users = $pdo->prepare("SELECT users.id, users.name, users.email, users.image, users.estado, roles.rol FROM users JOIN roles ON users.rol_id = roles.id");
...
<div class="container">
        ...
        <table id="myTableUsers" class="table table-striped table-bordered table-sm">
            ...
            <tbody id="userTableBody">
                ...
                <tr>
                    ...
                    <td class="estado"><p class="<?= $estado_class; ?>"><?= $row['estado']; ?></p></td>
                    <td>
                        <button class='btn btn-primary' onclick="verImagen(<?= $row['id']; ?>, 'uploaded_img/<?= $row['image']; ?>', '<?= $row['name']; ?>', '<?= $row['email']; ?>', '<?= $row['rol']; ?>', '<?= $row['estado']; ?>')"><i class='fa-solid fa-eye'></i></button>
                        <button class='btn btn-secondary' onclick="editarUsuario(<?= $row['id']; ?>, '<?= $row['name']; ?>', '<?= $row['email']; ?>', '<?= $row['rol']; ?>', 'uploaded_img/<?= $row['image']; ?>', '<?= $row['estado']; ?>')"><i class='fa-solid fa-pen-to-square'></i></button>
    ...
    window.verImagen = function(id, image, name, email, rol, estado) {
        $('#detalleID').text(id);
    ...
    window.editarUsuario = function(id, name, email, rol, image, estado) {
        $('#edit_id').val(id);
        $('#edit_name').val(name);
